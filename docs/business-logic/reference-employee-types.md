...
| `manager_trading` | Заведующая (торгует) | `DailyRevenueEntry.shiftType`; при включённом `fiveDayViaAttendance` — **оба** источника одновременно, не в одну дату (`AttendanceShift` от `shiftRate`, фикс. ставка без деления на календарь — как `seller_five_day_fixed`) | `/users` | да |
...
- **`FIVE_DAY_VIA_ATTENDANCE_TYPES`** = `{seller, manager_trading}` — единственные два типа, у которых `Employee.fiveDayViaAttendance` вообще что-то значит: включает учёт пятидневных дней через `AttendanceShift`. Для `manager_trading` чекбокс — на `/users` (там же, где остальные её переключатели); для `seller` — на `/employees`. У `manager_fixed`/`pharmacy_manager`/`office`/`cleaner` поле неприменимо (они и так только по табелю). **Важно:** у `seller` и `manager_trading` флаг даёт разную формулу и разное поведение источников смен — см. следующий пункт.
- **Поведение флага различается по типу — и формулой, и источниками смен.** У `seller` это переключатель "всё или ничего" по источнику: включив `fiveDayViaAttendance`, продавец на весь месяц теряет возможность получить смену в записи выручки (`canGetRevenueShift` → `false`), только табель, оплачиваемый по формуле `baseSalary/workingCalendarDays × attendanceCount`. У `manager_trading` — иначе с обеих сторон: смена в выручке остаётся разрешённой (`canGetRevenueShift` → `true` даже при включённом флаге, смешанный режим как у `seller_five_day_fixed`, не на одну и ту же дату), а табельные дни оплачиваются **фиксированной ставкой** `shiftRate × attendanceCount`, без деления на календарь — заведующая, которая торгует, набирает эти дни как любой сотрудник на фиксированной ставке, а не как оклад-зависимый табельный тип. Заведующей с оплатой от оклада/календаря без личных смен вообще соответствует отдельный тип `manager_fixed`. За разрешение обоих источников одновременно отвечает внутренняя `FIVE_DAY_VIA_ATTENDANCE_BLOCKS_SHIFTS_TYPES = {seller}` (не экспортируется, видна только через поведение `canGetRevenueShift`).
