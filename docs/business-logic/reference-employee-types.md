...
- **`MANAGER_TYPES`** = `{manager_trading, manager_fixed}` — оба получают 10%-долю от `pharmaBonus` управляемых аптек (`MANAGER_BONUS_SHARE_PERCENT = 0.1`). Ни один из них никогда не получает личную `revenuePremium` (та, что у продавца, — 1.5% от выручки смены сверх 200k/300k) — заведующие премируются только за бонусы/долю/доплату/лестницу, не за личную выручку. Лестничная премия по выручке аптеки (`Pharmacy.managerPremium*`) обязательна у `manager_fixed` и опциональна у `manager_trading` (флаг `ladderPremiumEnabled`) — см. [reference-salary-formulas.md](reference-salary-formulas.md). `pharmacy_manager` в группу `MANAGER_TYPES` не входит — у него нет 10%-доли, а лестничная премия опциональна (флаг `managerPremiumEnabled`).
