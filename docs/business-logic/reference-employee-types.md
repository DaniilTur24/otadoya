...
- **`MANAGER_TYPES`** = `{manager_trading, manager_fixed}` — оба получают 10%-долю от `pharmaBonus` управляемых аптек (`MANAGER_BONUS_SHARE_PERCENT = 0.1`) и лестничную премию по выручке аптеки (`Pharmacy.managerPremium*`). `pharmacy_manager` в эту группу **не входит** — у него нет 10%-доли, а лестничная премия опциональна (флаг `managerPremiumEnabled`).
