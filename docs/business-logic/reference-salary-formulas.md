...
### Суточная смена через полночь (`full_day_cont`)

Суточная смена начинается в один календарный день, а заканчивается утром следующего, тогда как касса снимается по календарным дням — то есть выручка одной смены попадает в **две** записи `DailyRevenueEntry`. Вторая запись помечается `shiftType = 'full_day_cont'` (константа `SHIFT_TYPES.full_day_cont`, проверка `isShiftContinuation()` в `src/lib/shift-types.ts`):

- в `fullDayShiftsCount` **не** идёт — оклад за смену начисляется один раз по первой записи;
- её выручка **идёт** в `revenueFullDayShifts` и в базу премии той же смены, поэтому премия считается по полным суткам, а порог `300000` берётся на одну смену, а не на две;
- дату **не занимает**: сдав сутки утром, сотрудник в тот же день может выйти на пятидневку по табелю или на дневную смену (см. `validateUniqueShift`, `validateNoShiftOnDate`, `validateNoAttendanceOnDate`).

Если продолжение попало в месяц, где самой суточной смены нет (сутки с 31-го на 1-е), его выручка в премию не засчитывается — иначе премия начислилась бы вообще без порога. Первая половина смены уже посчитана в своём месяце.

Заведующая этот тип не выбирает вручную: форма внесения выручки сама видит вчерашнюю суточную смену того же сотрудника и спрашивает, продолжение это или новые сутки (см. [howto-enter-daily-revenue.md](howto-enter-daily-revenue.md)).
