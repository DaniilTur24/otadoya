...
```
avgDayRevenue     = Σ(выручка всех подтверждённых смен «день» аптеки за месяц)  / кол-во таких смен
avgFullDayRevenue = Σ(выручка всех подтверждённых смен «сутки» аптеки за месяц) / кол-во таких смен
