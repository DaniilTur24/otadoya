...
1a. Отказывает (400), если в месяце есть записи выручки «на проверке» — в снимок они не вошли бы, а подтвердить их после закрытия уже нельзя; в ошибке — их число и сумма. Сначала подтвердить или удалить.
...
После закрытия:
- Новая запись выручки (`POST /api/revenue`) или изменение даты существующей в этот месяц отклоняется — 423.
- Редактирование/удаление/подтверждение существующих записей этого месяца блокируется (`PUT`/`DELETE /api/revenue/[id]`, `POST /api/revenue/[id]/approve` вернут 423). То же для подтверждения/отклонения строк расходов из файлов (`/api/expenses/[id]/approve|reject`).
...
- Больше ничего не трогает. `excludedFromReport` — отдельный, не связанный с закрытием месяца
  инструмент бухгалтера (вручную вычеркнуть ошибочную/дублирующую запись из отчёта и зарплаты
  через `PUT /api/revenue/[id]`, фильтр `excludedFromReport: false` в `computeMonthlyData()` и
  в расчёте зарплаты) — открытие месяца обратно не должно отменять это решение.
