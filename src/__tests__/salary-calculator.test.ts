...
// ─── смешанный график и второй оклад ─────────────────────────────────────────

/**
 * Один и тот же человек в одни дни выходит на суточную/дневную смену, в другие — по
 * пятидневке. Сменная часть считается от baseSalary, пятидневная — от отдельного оклада
 * fiveDaySalary; если второй оклад не задан, обе части считаются от baseSalary, как раньше.
 */
describe('смешанный график (workSchedule = mixed)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockBonuses(0);
    mockCalendar(22);
  ...
  it('платит и за смены, и за дни табеля — каждую часть по своему окладу', async () => {
    vi.mocked(prisma.employee.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 1, name: 'Айгуль Смакова', baseSalary: 190000, fiveDaySalary: 260000,
      employeeType: 'seller', workSchedule: 'mixed', pharmacies: [],
    });
    mockShifts([
      { shiftType: 'day', cashRevenue: 10000, terminalRevenue: 0, kaspiRevenue: 0 },
      { shiftType: 'full_day', cashRevenue: 10000, terminalRevenue: 0, kaspiRevenue: 0 },
    ]);
    vi.mocked(prisma.attendanceShift.count as ReturnType<typeof vi.fn>).mockResolvedValue(11);

    const result = await calculateEmployeeMonthlySalary(1, 5, 2026);

    expect(result!.dayShiftsCount).toBe(1);
    expect(result!.fullDayShiftsCount).toBe(1);
    expect(result!.attendanceShiftsCount).toBe(11);
    expect(result!.salaryFromDayShifts).toBeCloseTo(190000 / 15, 5);
    expect(result!.salaryFromFullDayShifts).toBeCloseTo(190000 / 10, 5);
    // Пятидневная часть идёт от второго оклада, а не от baseSalary
    expect(result!.salaryFromFiveDayShifts).toBeCloseTo((260000 / 22) * 11, 5);
    expect(result!.workSchedule).toBe('mixed');
  });

  it('без второго оклада пятидневная часть считается от основного', async () => {
    vi.mocked(prisma.employee.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 1, name: 'Айгуль Смакова', baseSalary: 190000, fiveDaySalary: null,
      employeeType: 'seller', workSchedule: 'mixed', pharmacies: [],
    });
    mockShifts([]);
    vi.mocked(prisma.attendanceShift.count as ReturnType<typeof vi.fn>).mockResolvedValue(10);
    ...
    const result = await calculateEmployeeMonthlySalary(1, 5, 2026);
    expect(result!.salaryFromFiveDayShifts).toBeCloseTo((190000 / 22) * 10, 5);
    expect(result!.fiveDaySalary).toBe(190000);
  });

  it('второй оклад применяется и к заведующей на пятидневке', async () => {
    vi.mocked(prisma.employee.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 4, name: 'Гульнара Ким', baseSalary: 190000, fiveDaySalary: 240000,
      employeeType: 'manager_fixed', workSchedule: null, pharmacies: [],
      ladderPremiumEnabled: false, managerBonusShareEnabled: false,
    });
    vi.mocked(prisma.attendanceShift.count as ReturnType<typeof vi.fn>).mockResolvedValue(20);

    const result = await calculateEmployeeMonthlySalary(4, 5, 2026);
    expect(result!.workSchedule).toBe('five_day');
    expect(result!.salaryFromFiveDayShifts).toBeCloseTo((240000 / 22) * 20, 5);
  });
});

describe('источник отработанных дней зависит от графика, а не от типа', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockBonuses(0);
    mockCalendar(22);
  ...
  // Раньше manager_fixed считался отдельной функцией, которая записи выручки не читала вовсе.
  // После слияния это поведение должно сохраниться, иначе такому сотруднику вдруг начнут
  // начисляться pharmaBonus с записей, где он просто указан как сотрудник смены.
  it('чисто табельный график не читает записи выручки', async () => {
    vi.mocked(prisma.employee.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 4, name: 'Гульнара Ким', baseSalary: 190000, employeeType: 'manager_fixed',
      workSchedule: null, pharmacies: [], ladderPremiumEnabled: false, managerBonusShareEnabled: false,
    });

    await calculateEmployeeMonthlySalary(4, 5, 2026);
    expect(prisma.dailyRevenueEntry.findMany).not.toHaveBeenCalled();
  });

  it('чисто сменный график не читает табель', async () => {
    vi.mocked(prisma.employee.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 1, name: 'Айгуль Смакова', baseSalary: 190000, employeeType: 'seller',
      workSchedule: null, pharmacies: [],
    });

    await calculateEmployeeMonthlySalary(1, 5, 2026);
    expect(prisma.attendanceShift.count).not.toHaveBeenCalled();
  });

  it('смешанный график читает оба источника', async () => {
    vi.mocked(prisma.employee.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 1, name: 'Айгуль Смакова', baseSalary: 190000, employeeType: 'seller',
      workSchedule: 'mixed', pharmacies: [],
    });

    await calculateEmployeeMonthlySalary(1, 5, 2026);
    expect(prisma.dailyRevenueEntry.findMany).toHaveBeenCalled();
    expect(prisma.attendanceShift.count).toHaveBeenCalled();
  });
});
