...
Only `status: 'approved'` records count. `totalSalary` can go negative if advances exceed earnings.

**Where worked days come from is `Employee.workSchedule`, not `employeeType`.** `seller`/`manager_trading`/`manager_fixed`/`pharmacy_manager` all run through one function (`calculateScheduleBasedSalary`); the schedule picks the source: `shift` = revenue-entry shifts only, `five_day` = `AttendanceShift` only, `mixed` = both at once (the same person works some days on a day/24h shift and others on a five-day schedule). `workSchedule = NULL` means "not chosen explicitly" and is derived by `resolveWorkSchedule()` so every pre-existing card keeps its old formula: `ATTENDANCE_BASED_TYPES` → `five_day`; the legacy `fiveDayViaAttendance` flag → **`mixed`** (not `five_day` — those employees' salary already read revenue entries for `pharmaBonus`, unlike `manager_fixed`); everything else → `shift`. `Employee.fiveDaySalary` is a second, optional oklad applied to the five-day part only; NULL falls back to `baseSalary`. One calendar day can never land in both channels — a shift on a date with an attendance mark (and vice versa) is rejected with 409 (`validateNoAttendanceOnDate` / `findShiftOnDate` in `src/lib/attendance-validation.ts`). `USER_LINKED_TYPES` (`manager_trading`, `manager_fixed`, `pharmacy_manager`) are included in `calculateAllEmployeesSalaries()` even with zero records, since their allowance/premium accrues regardless of personal shifts.
...
For everyone whose schedule reads the timesheet (`usesAttendance()` — i.e. `five_day` or `mixed`), each worked day is marked as one `AttendanceShift` row (unique on `employeeId` + `date`; `pharmacyId` optional — empty for office). Managers can only POST attendance for their own pharmacies (`getManagerPharmacyIds`); GET is scoped the same way. `getEmployeeMonthlyAttendance()` reads these for salary calculation.
